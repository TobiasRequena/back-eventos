...
async function crearEsquema(datos, trx = db) {
  const [esquema] = await trx('esquema_grupos_trabajo')
    .insert({
      ...
      criterio_tanda_atributo: datos.criterioTandaAtributo ? JSON.stringify(datos.criterioTandaAtributo) : null,
      modo_tamano: datos.modoTamano,
      valor_tamano: datos.valorTamano,
      balanceo_atributo: datos.balanceoAtributo ? JSON.stringify(datos.balanceoAtributo) : null,
      filtro_elegibilidad: datos.filtroElegibilidad ? JSON.stringify(datos.filtroElegibilidad) : null,
      modo_nombrado: datos.modoNombrado,
